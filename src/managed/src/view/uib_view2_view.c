...
void uib_view2_config_CIRCLE_360x360_portrait() {
	...
	if(vc->grid1) {
		...
		if (!vc->pb_set) {
			vc->pb_set = eext_circle_object_progressbar_add(vc->grid1, uib_views_get_instance()->get_window_obj()->circle_surface);
		}
		if (vc->pb_set) {
			eext_rotary_object_event_activated_set(vc->pb_set, EINA_TRUE);
			evas_object_size_hint_align_set(vc->pb_set, -1.0, -1.0);			evas_object_size_hint_weight_set(vc->pb_set, 1.0, 1.0);			eext_circle_object_value_min_max_set(vc->pb_set,0,100);
			eext_circle_object_value_set(vc->pb_set,0);
			evas_object_show(vc->pb_set);
		}
		elm_grid_pack(vc->grid1, vc->button1, 100, 100, 800, 80);		elm_grid_pack(vc->grid1, vc->button2, 100, 850, 800, 80);		elm_grid_pack(vc->grid1, vc->box1, 0, 265, 1000, 435);		elm_grid_pack(vc->grid1, vc->pb_set, 0, -2, 1000, 1003);		evas_object_show(vc->grid1);
	}
}
